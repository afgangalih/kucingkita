...
import { createBrand } from "@/app/(admin)/admin/brand/_actions/brand-actions";
...
export function AddBrandDialog({ onBrandCreated }: AddBrandDialogProps) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (res.error) {
        ...
        setOpen(false);
        if (onBrandCreated) onBrandCreated(res.data as Brand);
        router.refresh();
      }
    } catch {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="rounded-4xl border-none sm:max-w-md shadow-2xl p-10 bg-white">
        <DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-8 pt-6">
          ...
          >
            {isPending ? <Loader2 className="h-5 w-5 animate-spin" /> : "KONFIRMASI DATA"}
          </Button>
        </form>
      </DialogContent>
    </Dialog>
